...
int main()
{
    ...
    for (int N : Ns)
    {
        ...
        double gain_t  = (bRes.time_ms  > 0) ? nb_time_ms / bRes.time_ms : 999.0;
        // ss_batch=0 signifie pre-filtre clair a elimine toutes les paires non-coplanaires
        // → gain theoriquement infini ; on affiche le gain SS par rapport au nobatch
        double gain_ss = (bRes.ss_count > 0) ? (double)nb_ss / bRes.ss_count
                       : (nb_ss > 0 ? 999.0 : 1.0);
