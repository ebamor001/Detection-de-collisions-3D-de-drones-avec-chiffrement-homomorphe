...
# ── Config ────────────────────────────────────────────────────────────────────
PORT        = 8765
BINARY_PATH = os.path.join(os.path.dirname(__file__), "build", "drone_collision")
DATA_DIR    = os.path.join(os.path.dirname(__file__), "data")

# ── Helpers ───────────────────────────────────────────────────────────────────

def path_to_line(points):
      z = int(points[0][2]) if len(points[0]) >= 3 else 0
      return "".join(f"({int(p[0])},{int(p[1])},{z})" for p in points)
