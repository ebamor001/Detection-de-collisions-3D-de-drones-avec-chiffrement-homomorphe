...
public:
    ...
    // Charge un contexte externe (contexte de Bob reçu par Alice).
    // sk est requis pour regénérer les clés de scheme switching.
    // Note : en production, utiliser multi-key FHE pour éviter de partager sk.
    void   loadContext(CryptoContextCKKS        externalCC,
                       const PublicKey<DCRTPoly>& pk,
    ...
    std::vector<double> decryptVector(const CiphertextCKKS& ct, uint32_t n = 0);
