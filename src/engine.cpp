...
void CryptoEngine::initialize(const Config& cfg) {
    ...
    // config.ringDim = 8192;
    // apres batching
    config.multDepth    = 17;   // 20→17 : suffisant (2 mult orient + SS interne)
    config.scaleModSize = 50;
    config.ringDim      = 8192;
...
CryptoEngine::CiphertextCKKS CryptoEngine::compareGtZeroPacked(
    ...
    checkSwitchingReady();
    // EvalCompareSchemeSwitching exige numSlots == config.switchValues (valeur du setup).
    // kSlots <= config.switchValues est garanti par l'appelant (processGroup chunk).
    (void)kSlots;
    uint32_t numSlots = config.switchValues;
    auto zero = cc->EvalSub(xPacked, xPacked);
    return cc->EvalCompareSchemeSwitching(zero, xPacked, numSlots, numSlots);
}
